...
///
/// Matches Dart's `A2uiValidator`.
public enum A2uiValidator {

    /// Valid top-level action keys.
    ...
    /// Validates a single raw A2UI message JSON.
    ...
    private static func validateDataModelUpdate(_ dmu: JsonMap, errors: inout [String]) {
        ...
        if !(dmu["contents"] is [Any]) {
            errors.append("dataModelUpdate missing or invalid contents array")
        }
    }
...
/// Convenience function matching Dart's `validateA2uiJson`.
/// Validates and logs warnings for A2UI message errors.
